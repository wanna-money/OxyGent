...
- **func_filter**：MAS 级钩子，在处理之前修改传入的 payload。它接收并返回 payload 字典。用于请求增强（添加元数据、规范化字段）。
- **func_interceptor**：MAS 级钩子，可以短路请求处理。如果返回非 `None` 值，该值会立即返回给客户端，请求不会到达任何智能体。用于认证和授权。
