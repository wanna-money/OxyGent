...
- 包含技能定义文件的 `.oxygent/skills` 目录（或包含技能文件夹的父目录）
...
- `skills=[".oxygent/skills"]` -- 指向技能目录的路径列表。每个路径可以是单个技能文件夹或包含多个技能文件夹的父目录。
...
- **技能目录** -- 技能文件存储在 `.oxygent/skills`（或任何指定路径）中。每个技能通常定义一个任务，包含指令、所需工具和执行步骤。
...
3. `SkillAgent` 扫描 `.oxygent/skills` 目录，发现可用的技能定义，并在响应中列出。
