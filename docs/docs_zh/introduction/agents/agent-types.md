...
| `name`            | str   | 必填                          | 智能体名称        |
| `desc`            | str   | 必填                          | 智能体描述        |
| `llm_model`       | str   | 必填                          | 使用的语言模型标识符 |
| `prompt`          | str   | `"You are a helpful assistant."` | 系统提示词        |
| `short_memory_size` | int | 继承自 LocalAgent              | 短期记忆大小      |
...
- `max_react_rounds: int`：最大 ReAct 轮数
- `trust_mode: bool`：是否[提供响应元数据](../advanced/trust-mode.md)
- `func_parse_llm_response: Optional[Callable[[str], LLMResponse]]`：[处理 LLM 输出](../advanced/handle-output.md)
