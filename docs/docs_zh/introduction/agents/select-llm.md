...
| 100+ 供应商统一接入（Anthropic、Bedrock、Vertex 等） | `oxy.LiteLLM` | `model_name`（如 `anthropic/claude-sonnet-4-20250514`） |
| Ollama 本地部署模型 | `oxy.HttpLLM` | `base_url`（不传 api_key） |
| HuggingFace 本地模型 | `oxy.LocalLLM` | `model_path`, `device_map` |
| Ray Actor 本地模型 | `oxy.ActorLLM` | `actor_llm`, `actor_llm_timeout` |
...
[LiteLLM](https://github.com/BerriAI/litellm) 提供了一个统一的接口，可以接入 100+ LLM 供应商。使用前请先安装：`pip install litellm`。
...
LiteLLM 根据 `model_name` 前缀自动路由到对应的供应商：
...
当未提供 `api_key` 时，LiteLLM 会自动使用供应商对应的环境变量（`ANTHROPIC_API_KEY`、`OPENAI_API_KEY` 等）。您也可以通过 `base_url` 指向 LiteLLM 代理服务器。

> LiteLLM 需要额外安装：`pip install litellm`，不包含在默认依赖中。
...
## 使用 Ray Actor 模型（ActorLLM）

如果你有通过 Ray Actor 部署的模型，可以使用 `oxy.ActorLLM`：
...
> ActorLLM 需要安装 Ray 或兼容的 `ray_adapter` 模块。Actor 必须实现 `agent_generate_one_step()` 方法。
...
- [LiteLLM 示例](../../examples/llms/demo_litellm.md) — 通过 LiteLLM 使用任意 LLM 供应商
