...
### Step 1: Extract Historical Call Records from the Database

First, you need to retrieve historical call records from the database. Here is an example of using `es_client` to retrieve data from Elasticsearch:

```python
es_response = await mas.es_client.search(...)  # Replace with actual database
```

### Step 2: Build the Sample Structure

Then, you need to structure the data into the training sample format:
...
### Step 3: Use `sft_agent` for Automatic Labeling

Through `sft_agent`, you can automatically label and generate training data.

### Step 4: Write to Training Dataset File

Finally, write the generated training samples to a file for subsequent use:

```python
with open(to_jsonl_path, "w") as f:
    f.write("\n".join(datasets))  # The saved file can be directly used for the next round of training
```
...
- [GRPO Resampling Example](../../examples/advanced/demo_grpo_resample.md) -- Demonstrates how to use GRPO for multi-agent joint training sampling
- [Evaluation and Evolution Example](../../examples/agents/demo_evaluate_and_evolve.md) -- Demonstrates how to evaluate agent performance and automatically evolve and optimize
