...
- A `.oxygent/skills` directory with skill definition files (or a parent directory containing skill folders)
...
| `skill_agent` | `SkillAgent` | `llm_model="default_llm"`; `tools=["view_text_file", "execute_shell_command"]`; `skills=[".oxygent/skills"]` |
...
- `skills=[".oxygent/skills"]` -- a list of paths pointing to skill directories. Each path can be a single skill folder or a parent directory containing multiple skill folders.
...
- **Skills directory** -- skill files are stored in `.oxygent/skills` (or any specified path). Each skill typically defines a task with instructions, required tools, and execution steps.
...
3. The `SkillAgent` scans the `.oxygent/skills` directory, discovers available skill definitions, and lists them in its response.
